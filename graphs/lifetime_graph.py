...
def get_evaluation(solution, 
                   ...
                   verbose=1):
    global lifespan_list, timesteps_list
    ...
    for seed in seeds:
        logger.info(f"Using seeds: {seed}")
        ...
        # plt.close()
        # timesteps_list.pop()
        # lifespan_list[-1].append(lifespan_list[-2])
        lifespan_list[-1].append(lifespan_list[-2])

    # return OBJECTIVE, lifespan_list, timesteps_list
    return OBJECTIVE

def getLifespanList():
    print('lfiespan length', len(lifespan_list))
    return lifespan_list

def getTimestepsList():
    print('timespte length', len(timesteps_list))
    return timesteps_list


def get_average_lifetime(fleet, current_timestep):
    """
    Calculate the average lifespan of servers in the fleet.

    Parameters:
    - fleet: DataFrame containing server information including their deployment time.
    - current_timestep: The current time step in the simulation.

    Returns:
    - average_lifetime: The average operational lifetime of the servers.
    """
    if fleet.empty:
        return 0

    # Calculate the lifespan of each server
    fleet['lifespan'] = current_timestep - fleet['deployment_time']
    
    # Average the lifespan across all servers
    average_lifetime = fleet['lifespan'].mean()
...
def evaluation_function(solution, 
    ...
    # EVALUATE SOLUTION
    try:
        # evaluate_and_plot_for_seeds(solution, demand, datacenters, servers, selling_prices)
