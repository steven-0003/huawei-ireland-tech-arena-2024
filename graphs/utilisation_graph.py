...
def get_evaluation(solution, 
    ...
    seeds = [1741, 3163, 6053, 2237, 8237, 8933, 4799, 1061, 2543, 8501]
    for seed in seeds:
        ...
        plt.plot(timesteps_list, utilisation_list, marker='o', linestyle='-', color='b')
        plt.xlabel('Time-Step')
        plt.ylabel(f'Utilisation for seed {seed}')
        plt.title('Utilisation vs Time-Step for seed {}'.format(seed))
        plt.grid(True)
        plt.tight_layout()
        # plt.savefig(f'lifetime_{seed}.png')
        plt.show()
        plt.close()
